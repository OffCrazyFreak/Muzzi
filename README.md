...
| `hints_resolve` | turns links you paste into metadata, via yt-dlp. A wholly decorative bracketed group is removed whole, so `(Lyrics/Tekst)` cannot leave its slash behind, while `AC/DC` and `(Payphone Parody)` are kept. A pasted search is answered too, but its first result is scored below the auto bar and lands in review, because nobody picked it |
